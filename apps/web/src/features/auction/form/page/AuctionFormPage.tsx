...
const AuctionFormPage = async ({ auctionId }: AuctionFormPageProps) => {
  ...
  const userId = userInfo!.id; //NOTE - 미들웨어에서 비로그인시 메인 페이지로 리다이렉트 함
  const addressId = userInfo!.address_id; //NOTE - 미들웨어에서 주소가 없을 경우, 주소 등록 페이지로 리다이렉트 함
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      ...
      <AuctionForm auctionIdParam={isEditing ? auctionId : undefined} userId={userId} addressId={addressId} />
    </HydrationBoundary>
  );
};
